...
Robust, one-click import of FreeCAD (.fcstd) files into Blender.
Preserves assembly hierarchy, maps coordinates to Blender's Z-Up system,
converts triangulated OBJ output into clean BMesh N-gons, and supports
quick per-file reloads without losing the scene setup.
"""
...
def auto_find_freecad_path():
    ...
    # ----------------------------------------------------------------- Windows
        ...
        fc_subfolders = [
            "FreeCAD", "FreeCAD 1.1", "FreeCAD 1.1.1", "FreeCAD 1.0",
            "FreeCAD 0.22", "FreeCAD 0.21", "FreeCAD 0.20",
        ]
...
def get_freecad_lib_path(freecad_path):
    ...
    # macOS: Contents/Resources/bin/python -> Contents/Resources/lib
    if "Resources/bin" in freecad_path:
        lib_path = os.path.join(os.path.dirname(dirname), "lib")
        if os.path.exists(lib_path):
            return lib_path
    # Windows/Linux: FreeCAD/bin/python.exe -> FreeCAD/lib
    lib_path = os.path.join(os.path.dirname(dirname), "lib")
    if os.path.exists(lib_path):
        return lib_path
    return ""
...
def _build_subprocess_env(temp_dir):
    """Build an isolated environment for FreeCAD subprocess calls.
    Redirects all FreeCAD user paths to temp to prevent permission errors."""
    env = os.environ.copy()
...
def _safe_script_path(exporter_script, temp_dir):
    """Copy exporter script to temp dir to avoid spaces-in-path on Windows.
    Returns the safe copy path, or the original if copying fails."""
    safe = os.path.join(temp_dir, "fcstd_to_obj.py")
    try:
        shutil.copy2(exporter_script, safe)
        return safe
...
def _cleanup_temp_files(*paths):
    ...
    for f in paths:
        try:
            ...
            print(f"FreeCAD Imparator | Temp cleanup failed: {e}")
...
def clean_imported_meshes(objects):
    ...
    for obj in objects:
        ...
        try:
            ...
            try:
                # 0.0017 rad ≈ 0.1° — dissolves coplanar tris into flat N-gons
            ...
            bm.free()  # always release BMesh memory, even on exception
...
def reconstruct_hierarchy(json_path, imported_obs, import_coll):
    ...
    has_hidden = (any(c.get("hidden", False) for c in containers) or
                  any(m.get("hidden", False) for m in meshes))
    hidden_coll = None
    if has_hidden:
    ...
    def get_clean_name(name):
        """Strip TempMesh_ prefix and Blender .001 duplicate suffix."""
    ...
    for c in containers:
        ...
        pos = c["position"]
        rot = c["rotation"]
        empty = bpy.data.objects.new(c["label"], None)
        ...
        target_coll = hidden_coll if (c.get("hidden", False) and hidden_coll) else import_coll
        target_coll.objects.link(empty)
        created_empties[c_name] = empty

    # Evaluate matrices before parenting
    bpy.context.view_layer.update()

    # 2. Establish container hierarchy
    for c in containers:
        parent_name = c["parent_name"]
        if parent_name and parent_name in created_empties:
            empty = created_empties[c["name"]]
    ...
    # 3. Map, rename, and apply materials to imported mesh objects
    mesh_map = {}
    for obj in imported_obs:
        ...
        for m in meshes:
            ...
            if m.get("hidden", False) and hidden_coll:
...
class FreeCADImportItem(PropertyGroup):
    ...
    deflection: FloatProperty(
        name="Deflection", default=0.1, min=0.001, max=10.0,
        description="Tessellation linear deflection (mm)"
    )
    scale: EnumProperty(
        name="Scale",
...
class FreeCADAddonPreferences(AddonPreferences):
    ...
    freecad_path: StringProperty(
        ...
        default=auto_find_freecad_path(),
        subtype='FILE_PATH'
    )
...
class FCSTD_OT_auto_detect(Operator):
    ...
    bl_description = "Search common installation paths and system tools to locate FreeCAD automatically"
    bl_options = set()
...
class FCSTD_OT_import(Operator, ImportHelper):
    ...
    filter_glob: StringProperty(default="*.fcstd", options={"HIDDEN"})

    deflection: FloatProperty(
        name="Mesh Quality (Deflection)",
        description="Linear deflection for tessellation. Smaller = higher quality",
        default=0.1, min=0.001, max=5.0, subtype='DISTANCE'
    )
    ...
    def execute(self, context):
        ...
        if not os.path.exists(exporter_script):
            self.report({'ERROR'}, f"Exporter script not found: {exporter_script}")
            return {'CANCELLED'}

        exporter_script = _safe_script_path(exporter_script, temp_dir)

        cmd = [freecad_bin, exporter_script, self.filepath, temp_obj,
               str(self.deflection), str(self.scale)]
        ...
        try:
            process = subprocess.run(
                cmd,
                stdout=subprocess.DEVNULL,  # discard; only stderr matters
                stderr=subprocess.PIPE,
                text=True,
                env=_build_subprocess_env(temp_dir),
                check=False
            )
            if process.returncode != 0:
                err = (process.stderr or "Unknown error").strip()
                self.report({'ERROR'}, f"FreeCAD export failed: {err[:200]}")
                return {'CANCELLED'}
        except Exception as e:
        ...
        _cleanup_temp_files(temp_obj, temp_obj.replace(".obj", ".mtl"))

        if not import_success:
            self.report({'ERROR'}, "Blender failed to import the generated OBJ file.")
            return {'CANCELLED'}

        imported_obs = [bpy.data.objects[n] for n in (set(bpy.data.objects.keys()) - old_objects)]
        if not imported_obs:
            self.report({'WARNING'}, "Imported successfully, but no new 3D objects were found.")
            return {'FINISHED'}

        # Convert triangulated OBJ into clean Blender-native N-gons
        ...
        while bpy.data.collections.get(coll_name):
            coll_name = f"{base_name}.{counter:03d}"
            counter += 1
        ...
        # Reconstruct FreeCAD assembly hierarchy
...
class FCSTD_OT_reload(Operator):
    ...
    def execute(self, context):
        ...
        addon_dir = os.path.dirname(__file__)
        exporter_script = _safe_script_path(
            os.path.join(addon_dir, "fcstd_to_obj.py"), temp_dir
        )

        # item.scale is always a string (EnumProperty)
        cmd = [freecad_bin, exporter_script, filepath, temp_obj,
               str(item.deflection), item.scale]
        lib_path = get_freecad_lib_path(freecad_bin)
        if lib_path:
            cmd.append(lib_path)
        ...
        # Delete old objects in target collection
        coll = bpy.data.collections.get(collection_name)
        if coll:
            for obj in list(coll.all_objects):
                try:
                    ...
                    print(f"FreeCAD Imparator | Error removing old object: {e}")
        else:
...
class FCSTD_OT_smooth(Operator):
    ...
    bl_label = "Smooth"
    bl_description = "Shade smooth by angle (Auto Smooth) all meshes in selected or imported FreeCAD collections"
    bl_options = {"REGISTER", "UNDO"}
    ...
    def execute(self, context):
        ...
        if not meshes:
            self.report({'WARNING'}, "No meshes found. Select meshes or import a model first.")
            return {'CANCELLED'}
    ...
    def draw(self, context):
        ...
        if imports:
            for idx, item in enumerate(imports):
                ...
                if not bpy.data.collections.get(item.collection_name):
...
def unregister():
    ...
    if hasattr(bpy.types.Scene, "freecad_imports"):
